...
object SyntaxGen3 extends CodeGen[IO] {
  ...
  def templateApplicative(name: String, pair: String, end: String, functor: String, input: String)(params: Int): String = params match {
    case i =>
      ...
      s"""
         |  inline def $name$params($args): $functor[$output] = $input match {
  ...
  def templateMonad(
    ...
    params match {
      case i =>
        ...
        s"""
         |    inline def $name$params($args): $functor1[$output] = $input match {
         |      case $deconstruct =>
         |        $pair1(a, pa).flatMap { ar =>
         |          ($pairs).${if (i == 2) "map" else "mapN"}{ case $resultsMatch => $resultsTuple } <* $end($lastString)
         |        }
         |    }
         |""".stripMargin
    }
  ...
  override def file: Path = Path("./core/src/main/scala-3/net/andimiller/cats/parse/interpolator/package.scala")
  ...
  override def contents: Stream[IO, String] = {
    ...
    val pm       = templateMonad("pm", "pair", "pair0", "endParser0", "Parser", "Parser0", "partsSafeHead")(_)
